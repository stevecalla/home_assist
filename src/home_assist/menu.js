...
// Analogue audio is a DIFFERENT binary (rtl_fm, not rtl_433) and a different concern, so it gets
// its own script. Same dongle, so the same stop-the-collector rule applies inside it.
const AUDIO = 'src/home_assist/modules/water/audio.js';
...
const SECTIONS = [
  ...
  { label: 'RADIO — listen to it yourself', color: BLUE, items: [
    ...
    { label: 'Hopping endpoints — my window only', desc: 'Protocols 282/290 (the frequency-hopping Orions) on the collector\'s own 1.6 MHz window. Quick look — silence proves nothing here, a decode is real', bin: 'node', args: [LISTEN, 'hop'], cli: 'node ' + LISTEN + ' hop' },
    { label: 'Hopping endpoints — the Orion hop range', desc: 'THE test for 282/290. 904.4-924.6 MHz in 15 hops of 1.6 MHz — the range AND sample rate rtl_433 documents for Orion Endpoints. ~5 min a pass; run several. Ctrl-C prints every endpoint heard', bin: 'node', args: [LISTEN, 'hopsweep'], cli: 'node ' + LISTEN + ' hopsweep' },
    ...
    { label: 'Which Orion decoders are in this build?', desc: 'rtl_433 -R help, filtered to Orion. Reports 223 (the collector) and 282/290 (the hopping variants) separately. No dongle needed, does not touch the collector', bin: 'node', args: [LISTEN, 'check'], cli: 'node ' + LISTEN + ' check' },
  ]},
  ...
  { label: 'DATABASE', color: CYAN, items: [
    ...
    { label: 'Seed TEST neighbour meters (dev)', desc: 'Adds three obviously-fake meters (999000xx) so meter-access grants can be tested on a laptop that only ever hears one meter. Not for production', bin: 'node', args: ['src/home_assist/modules/water/seed_meters.js'], cli: 'node src/home_assist/modules/water/seed_meters.js' },
    { label: 'Remove the TEST meters', desc: 'Deletes exactly the three seeded ids. Leaves every real meter alone', bin: 'node', args: ['src/home_assist/modules/water/seed_meters.js', '--remove'], cli: 'node src/home_assist/modules/water/seed_meters.js --remove' },
    { label: 'List the meter registry', desc: 'Every meter this receiver knows about, real and seeded', bin: 'node', args: ['src/home_assist/modules/water/seed_meters.js', '--list'], cli: 'node src/home_assist/modules/water/seed_meters.js --list' },
